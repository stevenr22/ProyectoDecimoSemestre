...
<body>
                                                ...
                                                while ($arreglo = $senten->fetch_array()) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <?php if ($arreglo['cargo'] !== 'Administrador'): ?>
                                                                <button type="button" class="btn btn-info"
                                                                    onclick="modalActuRol('<?php echo $arreglo['id_rol'] ?>','<?php echo $arreglo['cargo'] ?>','<?php echo $arreglo['descripcion'] ?>');"
                                                                    id="celeste"><i class="fa-solid fa-pencil"></i></button>
                                                            <?php endif; ?>

                                                            <button type="button" class="btn btn-danger"
                                                                onclick="eliminar_rol('<?php echo $arreglo['id_rol'] ?>','<?php echo $arreglo['cargo'] ?>');"
    ...
    $("#formRegisRol").submit(function(e) {
        ...
        $.ajax({
            ...
            data: {
                nom_rol: nom_rol,
                decrip_rol: decrip_rol
            }, // Asegúrate de que los nombres coincidan con los de tu PHP
    ...
    $(document).ready(function() {
        $("#modalActuaRol").submit(function(e) {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    id_rol_act: id_rol_act,
                    nom_rol_actu: nom_rol_actu,
                    decrip_rol_actu: decrip_rol_actu
                }, // Enviar el ID
                success: function(response) {
                    if (response.status === 'success') {
                        Swal.fire({
    ...
    if (cargo === 'Administrador') {
        ...
        return; // Detiene la ejecución de la función si es Administrador
    }
            ...
            $.ajax({
                ...
                success: function(response) {
                    Swal.fire({
                        title: 'Eliminado',
                        icon: 'success',
                        text: 'El cargo ' + cargo + ' se ha eliminado con éxito!',
                        confirmButtonText: 'Ok',
                    }).then((result) => {
